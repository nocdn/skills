## Install

```bash
bunx skills add nocdn/skills --skill build-ios-apps
bunx skills add nocdn/skills --skill generate-commit-message
bunx skills add nocdn/skills --skill generating-llms-txt
bunx skills add nocdn/skills --skill no-ctx7
bunx skills add nocdn/skills --skill write-api-helper-route
bunx skills add nocdn/skills --skill inspect-code
```
